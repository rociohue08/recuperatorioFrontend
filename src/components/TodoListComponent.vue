...
<template>
  ...
  <div v-bind:class="{'dark': themeStore.isDark}" class="list-wrapper max-w rounded overflow-y-auto shadow-lg mt-10 p-4 transition ease-linear">
      <div class="px-6 py-4">
      ...
      <!-- envoltura de la tarea, insertar v-for aca! -->
        <div v-for="taskItem in task.data" :key="taskItem.id" class="wrapper relative group border-black my-2 transition ease-linear">

          <form v-on:submit.prevent>
              <div class="absolute top-3 sm:top-4 left-5">
                  <!-- aca aplicar directiva @click para cambiar estado de tarea -->
                  <div class="relative">
                      <input  @click="makeItComplete(taskItem)"
                          type="checkbox"
                      ...
                      <CompletedIcon  v-if="taskItem.completada" class="h-100 w-100 absolute left-0 top-0 text-green-600"/>

                    </div>
              </div>

              <!-- usar v-model para pasar el texto de la tarea en input y usar v-bind:class para modo oscuro -->
              <input v-model="taskItem.tarea"
              ...
              <div class="btns absolute right-0 top-0 py-2 sm:py-2.5 px-2 w-20 h-14 flex justify-around cursor-default">
                  ...
                  <button @click="task.removeTask(taskItem)" class="p-1 cursor-pointer">
      ...
      <div class="px-6 py-2 mt-5">
          ...
          <span @click="reactiveNewTask.complete" class="inline-block bg-gray-300 rounded px-3 py-1 text-sm font-semibold mr-2 mb-2 hover:bg-gray-500 cursor-pointer shadow-lg">Completas</span>
