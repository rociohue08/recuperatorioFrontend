...
<template>
  ...
  <div v-bind:class="{'dark': themeStore.isDark}" class="wrapper transition ease-linear">
        ...
        <label for="toggle" class="flex items-center justify-center cursor-pointer">
          <div class="relative">
            ...
            <div class="dot absolute left-1 top-1 bg-black w-6 h-6 flex items-center justify-center rounded-full transition">
              ...
              <MoonIcon v-if="!themeStore.isDark" class="w-4 h-4 text-white" />
              <SunIcon v-if="themeStore.isDark" class="w-full h-full text-yellow-500 p-1" />
            </div>
          </div>
          <!-- cambiar usar v-bind:class y atributo mode del ThemeState para cambiar el texto -->
          <div v-bind:class="{'dark': themeStore.isDark}" class="label-text ml-2 font-medium">Modo {{ themeStore.mode }}</div>
        </label>
      </div>
    </div>
      <!-- usar directiva v-bind:class para asinar clase class si isDark en el store es true -->
    <div v-bind:class="{'dark': themeStore.isDark}" class="img min-h-screen flex flex-col items-center transition">
    ...
    <div class="absolute bottom-0 w-full h-20 flex flex-col justify-center items-center bg-slate-500 text-xl font-semibold">
      <div class="unknowncode">
        <span class="font-light mx-2">by</span>unknown<span class="text-gray-200 font-normal">code</span>44
      </div>
      <p class="text-sm font-normal">Matias Orellana 2024</p>
      <a class="text-sm font-normal text-white cursor-pointer flex flex-row justify-around items-center" href="https://github.com/unknowncode44">
